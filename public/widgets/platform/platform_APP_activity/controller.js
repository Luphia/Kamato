Kamato.register.controller('appActivityCtrl', function ($scope, $http, $modal, ngDialog, $rootScope, $timeout, $routeParams) {
    ...
    var socket = io('https://' + location.hostname + '/' + $routeParams.APP, { autoConnect: false, secure: true });
    ...
    socket.on('summary', function (data) {
        if ($('.main-chart').length > 0 && start == false) {
            ...
            $scope.app_info[0].network.in = displayByte(data.current.in[0], 'B').join(' ');
            $scope.app_info[0].network.out = displayByte(data.current.out[0], 'B').join(' ');
    ...
    function Get_Data(num, nin, nout) {
        var x = nin[0];
        $scope.app_info[0].network.in = displayByte(x, 'B').join(' ');

        var xx = nin[1];
        var y = nout[0];
        $scope.app_info[0].network.out = displayByte(y, 'B').join(' ');
